...
export async function seedShortcuts(): Promise<void> {
  try {
    console.log('🌱 Seeding shortcuts...');
    for (const s of SHORTCUTS) {
      const id = s.title.replace(/\s+/g, '-').toLowerCase().slice(0, 36);
      await prisma.shortcut.upsert({
        ...
        create: {
          ...
          ...(s.gateYear !== undefined ? { gateYear: s.gateYear } : {}),
        },
      });
    }
    console.log(`✅ Shortcuts seeded (${SHORTCUTS.length})`);
  } catch (e) {
